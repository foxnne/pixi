...
const Atlas = @import("../storage/external.zig").Atlas;
...
pub const ProcessAssetsStep = struct {
    ...
    fn process(step: *Step, prog_node: *std.Progress.Node) !void {
        _ = prog_node;
        ...
        _ = std.fs.cwd().openDir(root, .{ .access_sub_paths = true }) catch |err| {
            std.debug.print("Not a directory: {s}, err: {}", .{ root, err });
            return;
        };
        // path passed is a directory
        const files: [][:0]const u8 = getAllFiles(self.builder.allocator, root, true);
        ...
        var assets_array_list = ArrayList(u8).init(self.builder.allocator);
        ...
        // Iterate all files
        for (files) |file| {
            try self.processFile(file, assets_writer, animations_output);
        }
    ...
    fn processFile(
        self: *ProcessAssetsStep,
        file: []const u8,
        assets_writer: ArrayList(u8).Writer,
        animations_output: []const u8,
    ) !void {
        ...
        const recursor = struct {
            fn search(alloc: std.mem.Allocator, directory: []const u8, recursive: bool, filelist: *std.ArrayList([:0]const u8)) void {
                ...
                while (iter.next() catch unreachable) |entry| {
                    if (entry.kind == .file) {
                        const name_null_term = std.mem.concat(alloc, u8, &.{ entry.name, "\x00" }) catch unreachable;
                        const abs_path = std.fs.path.join(alloc, &.{ directory, name_null_term }) catch unreachable;
                        filelist.append(abs_path[0 .. abs_path.len - 1 :0]) catch unreachable;
                    } else if (entry.kind == .directory) {
                        const abs_path = std.fs.path.join(alloc, &.{ directory, entry.name }) catch unreachable;
