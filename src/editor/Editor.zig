...
pub fn init(
    ...
    editor.colors.keyframe_palette = try Pixi.storage.Internal.Palette.loadFromFile(Pixi.assets.pear36_hex.path);
...
pub fn lateInit(core: *Core, app: *Pixi, editor: *Editor) !void {
    const theme_path = try std.fs.path.joinZ(app.allocator, &.{ Pixi.assets.themes, editor.settings.theme });
