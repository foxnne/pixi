...
pub fn draw(core: *Core) !void {
    ...
    if (imgui.beginChild("SettingsChild", .{
        ...
        imgui.pushItemWidth(imgui.getWindowWidth() - Pixi.app.settings.explorer_grip * Pixi.app.content_scale[0]);
        ...
        if (imgui.collapsingHeader(Pixi.fa.mouse ++ "  Input", imgui.TreeNodeFlags_Framed)) {
            ...
            imgui.pushItemWidth(Pixi.app.settings.explorer_width * Pixi.app.content_scale[0] * 0.5);
            if (imgui.beginCombo("Scheme", @tagName(Pixi.app.settings.input_scheme), imgui.ComboFlags_None)) {
                defer imgui.endCombo();
                if (imgui.selectable("mouse")) {
                    Pixi.app.settings.input_scheme = .mouse;
                }
                if (imgui.selectable("trackpad")) {
                    Pixi.app.settings.input_scheme = .trackpad;
                }
            }

            if (Pixi.app.settings.input_scheme == .trackpad) {
                _ = imgui.sliderFloatEx("Pan Sensitivity", &Pixi.app.settings.pan_sensitivity, 1.0, 25.0, "%.0f", imgui.SliderFlags_AlwaysClamp);
                _ = imgui.sliderFloatEx("Zoom Sensitivity", &Pixi.app.settings.zoom_sensitivity, 1, 200, "%.0f%", imgui.SliderFlags_AlwaysClamp);
            }

            if (builtin.os.tag == .macos) {
                if (imgui.checkbox("Ctrl zoom", &Pixi.app.settings.zoom_ctrl)) {
                    Pixi.app.allocator.free(Pixi.app.hotkeys.hotkeys);
                    Pixi.app.hotkeys = try Pixi.input.Hotkeys.initDefault(Pixi.app.allocator);
                }
            }
        ...
        if (imgui.collapsingHeader(Pixi.fa.th_list ++ "  Layout", imgui.TreeNodeFlags_None)) {
            ...
            _ = imgui.sliderFloatEx(
                "Info Height",
                &Pixi.app.settings.info_bar_height,
                18,
                36,
                "%.0f",
            ...
            _ = imgui.sliderFloatEx(
                "Sidebar Width",
                &Pixi.app.settings.sidebar_width,
                25,
                75,
                "%.0f",
                imgui.SliderFlags_None,
            ...
            _ = imgui.checkbox(
                "Show Rulers",
                &Pixi.app.settings.show_rulers,
            );

            _ = imgui.sliderFloatEx(
                "Explorer Title Align",
                &Pixi.app.settings.explorer_title_align,
                0.0,
                1.0,
                "%0.1f",
                imgui.SliderFlags_None,
        ...
        if (imgui.collapsingHeader(Pixi.fa.sliders_h ++ "  Configuration", imgui.TreeNodeFlags_None)) {
            ...
            _ = imgui.checkbox(
                "Dropper Auto-switch",
                &Pixi.app.settings.eyedropper_auto_switch_layer,
            );

            imgui.separator();

            imgui.text("Reference Window Opacity");
            _ = imgui.sliderFloatEx("##reference_window_opacity", &Pixi.app.settings.reference_window_opacity, 0.0, 100.0, "%.0f", imgui.SliderFlags_AlwaysClamp);

            imgui.separator();

            if (imgui.beginCombo("Compatibility", if (Pixi.app.settings.compatibility == .none) "None" else "LDtk", imgui.ComboFlags_None)) {
                defer imgui.endCombo();

                if (imgui.selectable("None")) Pixi.app.settings.compatibility = .none;
                if (imgui.selectable("LDtk")) Pixi.app.settings.compatibility = .ldtk;
            }
        ...
        if (imgui.collapsingHeader(Pixi.fa.paint_roller ++ "  Style", imgui.TreeNodeFlags_None)) {
            ...
            if (imgui.beginCombo("Theme", Pixi.app.settings.theme, imgui.ComboFlags_None)) {
                defer imgui.endCombo();
                try searchThemes();
            }
            imgui.separator();
